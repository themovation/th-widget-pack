...
class Astra_Target_Rules_Fields {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_hfe_get_posts_by_query', array( $this, 'aloha_hfe_get_posts_by_query' ) );
	}
	 ...
	 * @since  1.0.0
	 */
	function aloha_hfe_get_posts_by_query() {

		$Astra_Target_Rules_Fields = Astra_Target_Rules_Fields::get_instance(); check_ajax_referer( 'hfe-get-posts-by-query', 'nonce' );
		...
		foreach ( $post_types as $key => $post_type ) {
			...
			add_filter( 'posts_search', array( $Astra_Target_Rules_Fields, 'search_only_titles' ), 10, 2 );
